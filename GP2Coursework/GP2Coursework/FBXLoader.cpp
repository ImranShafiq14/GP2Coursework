...
void processAttribute(FbxNodeAttribute * attribute, GameObject * go)
{
	...
	{
		//get mesh
		Mesh * mesh = MeshManager::getManager().getMesh(std::string(attrName));
		//if we don't have it load and add
		if (!mesh)
		{
			Mesh * mesh = processMesh(attribute->GetNode()->GetMesh());
			MeshManager::getManager().addMesh(std::string(attrName), mesh);
		}
		go->setMesh(mesh);
		break;
	}
